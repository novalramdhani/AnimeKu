<x-app-layout title="Ani Cast - Seasons">
    <div class="container px-4 pt-16">
        <div class="now-anime">
            <h1 class="uppercase tracking-wider text-purple-600 text-lg font-semibold">Now Airing</h1>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                @foreach ($nowList as $now)
                    <div class="mt-8">
                        <a href="{{ route('anime.show', $now['mal_id']) }}"><img src="{{ $now['images']['jpg']['image_url'] }}" class="hover:opacity-75 transition case-in-out duration-150 h-80 w-80" alt="Poster"></a>
                        <div class="mt-2">
                            <a href="{{ route('anime.show', $now['mal_id']) }}" class="text-lg mt-2 text-white hover:text-gray-300">{{ $now['title'] }}</a>
                            <div class="flex items-center text-gray-300 text-sm mt-1">
                                ...
                                <span class="ml-1">{{ isset($now['year']) ? $now['year'] : 'Unknown' }}</span>
